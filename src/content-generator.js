  ...
  generateChallenge(lessonLevel, manualChars = []) {
    ...
    if (useRealContent) {
      ...
      if (contentPool.words.length) {
        availableContentTypes.push('words');
      }
      if (contentPool.abbrs.length) {
        availableContentTypes.push('abbrs');
      }
      if (contentPool.qcodes.length) {
        availableContentTypes.push('qcodes');
      }
      if (contentPool.phrases.length) {
        availableContentTypes.push('phrases');
      }
      if (contentPool.numbers.length) {
        availableContentTypes.push('numbers');
      }
      if (contentPool.bands.length) {
        availableContentTypes.push('bands');
      }
      if (contentPool.frequencies.length) {
        availableContentTypes.push('frequencies');
      }
